...
test.describe('page 9999: Login Page', () => {
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new LoginPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('TESTE - Log In'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 0 hidden)', async ({ page }) => {
    const po = new LoginPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P9999_PASSWORD', 'P9999_REMEMBER', 'P9999_USERNAME'], 9999);
  });
  ...
  test('every uniquely labeled .apx button exists (1 button)', async ({ page }) => {
    const po = new LoginPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Sign In'], [{ pageId: 9999, identifier: 'login' }]);
  });
});
