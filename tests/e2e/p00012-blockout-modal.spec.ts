 ...
 * (./p00012-blockout-modal.page.js), not raw testkit calls, so both stay in sync.
...
import { BlockoutModalPage } from './p00012-blockout-modal.page.js';

test.describe('page 12: Register Blockout [not auto-routable -- skipped]', () => {
  ...
  test('loads via alias URL with clean console' , async ({ page }) => {
    const po = new BlockoutModalPage(page);
  ...
  test('title matches metadata (normalized)', async ({ page }) => {
    const po = new BlockoutModalPage(page);
    await po.goto();
    expect(normalizeTitle(await page.title())).toBe(normalizeTitle('Register Blockout'));
  });

  test('every .apx pageItem exists in DOM/apex.item registry (3 items, 0 hidden)', async ({ page }) => {
    const po = new BlockoutModalPage(page);
    await po.goto();
    await expectItemsPresent(page, ['P12_START_DATE', 'P12_END_DATE', 'P12_REASON'], 12);
  });

  test('apex.item round-trip on P12_REASON', async ({ page }) => {
    const po = new BlockoutModalPage(page);
    await po.goto();
  ...
  test('every uniquely labeled .apx button exists (2 buttons)', async ({ page }) => {
    const po = new BlockoutModalPage(page);
    await po.goto();
    await expectButtonsPresent(page, ['Cancel', 'Save Blockout'], [{ pageId: 12, identifier: 'cancel' }, { pageId: 12, identifier: 'save-blockout' }]);
  });
});
